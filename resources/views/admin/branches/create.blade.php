            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required
                           ...
                           placeholder="e.g. CapaciPrint Downtown Main">
                ...
                <div>
                    ...
                    <input type="number" name="max_daily_jobs" value="{{ old('max_daily_jobs', 20) }}" min="1" max="1000" required
                ...
                <div class="sm:col-span-2">
                    ...
                    <select name="status" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm">
                        <option value="active" {{ old('status') === 'active' ? 'selected' : '' }}>Active (Accepting Print Jobs)</option>
                        <option value="maintenance" {{ old('status') === 'maintenance' ? 'selected' : '' }}>Maintenance (Partial Operations)</option>
                        <option value="inactive" {{ old('status') === 'inactive' ? 'selected' : '' }}>Inactive (Offline)</option>
                    </select>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="address" rows="3"
                              class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm"
                              placeholder="Building, Street, Barangay, City, Postal Code">{{ old('address') }}</textarea>
                </div>
            </div>
