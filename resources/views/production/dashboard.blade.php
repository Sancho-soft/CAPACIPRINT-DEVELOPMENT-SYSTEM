...
<div class="space-y-6 w-full max-w-7xl mx-auto">
    ...
    <div class="relative bg-cyber-card border border-cyber rounded-3xl p-6 sm:p-7 shadow-2xl overflow-hidden">
        ...
        <div class="relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            <div class="flex items-center gap-4 sm:gap-5">
                <div class="h-14 w-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 dark:border-cyan-500/30 text-cyan-700 dark:text-cyan-400 flex items-center justify-center text-2xl shadow-sm shrink-0 font-display font-black">
                ...
                <div>
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-500/10 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 dark:border-emerald-500/30 font-mono">
                            Shift Active
                        </span>
                    </div>
                    <p class="text-xs text-cyber-muted mt-1 leading-relaxed">
                        Operator: <strong class="text-cyber-main">{{ auth()->user()->name }}</strong> &middot; You have <strong class="text-cyan-700 dark:text-cyan-400 font-mono font-bold">{{ $inProductionCount }} active run(s)</strong> running on your assigned press lines.
                    </p>
                </div>
            </div>
            ...
            <div class="bg-cyber-sub/80 border border-cyber rounded-2xl p-4 min-w-[280px] w-full lg:w-auto shadow-sm">
                <div class="flex items-center justify-between text-xs mb-2">
                    ...
                    <span class="text-cyan-700 dark:text-cyan-400 font-bold font-mono">{{ $completedCount }} / {{ max(1, $assignedCount + $completedCount) }} Runs</span>
                </div>
                @php 
                ...
                <div class="w-full bg-cyber-base rounded-full h-2 overflow-hidden border border-cyber/60">
                    <div class="bg-gradient-to-r from-cyan-500 to-emerald-500 dark:from-cyan-400 dark:to-emerald-400 h-2 rounded-full transition-all duration-500 ease-out" style="width: {{ $pct }}%"></div>
                </div>
                <div class="flex items-center justify-between text-[10px] text-cyber-muted mt-2 font-medium">
                    ...
                    <span class="text-emerald-700 dark:text-emerald-400 font-bold font-mono">{{ $pct }}% Complete</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-cyber-card border border-cyber rounded-3xl p-5 sm:p-6 shadow-xl space-y-4">
        <div class="flex items-center justify-between border-b border-cyber/80 pb-3">
            ...
            <i class="fa-solid fa-print text-cyan-600 dark:text-cyan-400 text-sm"></i>
        </div>
            ...
            @foreach($pressMachines as $m)
                @php
                    $mBadge = match($m->status) {
                        'available'   => 'bg-emerald-500/10 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-500/20 dark:border-emerald-500/30',
                        'in_use'      => 'bg-cyan-500/10 dark:bg-cyan-500/15 text-cyan-700 dark:text-cyan-400 border-cyan-500/20 dark:border-cyan-500/30',
                        'maintenance' => 'bg-amber-500/10 dark:bg-amber-500/15 text-amber-800 dark:text-amber-400 border-amber-500/20 dark:border-amber-500/30',
                        default       => 'bg-rose-500/10 dark:bg-rose-500/15 text-rose-700 dark:text-rose-400 border-rose-500/20 dark:border-rose-500/30',
                    };
                @endphp
                <div class="p-4 rounded-2xl border border-cyber bg-cyber-sub/60 space-y-2 hover:border-slate-300 dark:hover:border-slate-700 hover:shadow-xs transition-all duration-200">
                    <div class="flex items-start justify-between gap-2">
                        <span class="font-bold text-cyber-main text-xs truncate max-w-[150px]">{{ $m->name }}</span>
                        <span class="px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-wider border {{ $mBadge }} font-mono shrink-0">
                    ...
                    <div class="text-[10px] text-cyber-sub flex justify-between pt-1 border-t border-cyber/40">
                        <span>Capacity:</span>
                        <span class="font-mono text-cyber-main font-semibold">{{ $m->jobs_per_day_capacity ?? 10 }} jobs/day</span>
                    </div>
                </div>
            @endforeach
        </div>
